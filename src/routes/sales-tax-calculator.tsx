  ...
  const getReportInput = useCallback((): CalcReportInput => {
    return buildCalcReportInput(
      "Sales Tax Calculator",
      {
        ...
        "Input Amount": `$${priceInput.toFixed(2)}`,
        "Tax Rate": `${taxRate}%`,
      },
      `$${result.totalPrice.toFixed(2)}`,
      {
        metrics: [
          { label: "Net Price (Pre-Tax)", value: `$${result.netPrice.toFixed(2)}` },
          { label: "Sales Tax Amount", value: `$${result.taxAmount.toFixed(2)}` },
          { label: "Gross Total (With Tax)", value: `$${result.totalPrice.toFixed(2)}` },
        ],
        formula:
          direction === "forward"
            ? "Tax = Net × (Rate / 100); Gross = Net + Tax"
            : "Net = Gross ÷ (1 + Rate / 100); Tax = Gross - Net",
        explanation: `At a ${taxRate}% tax rate, a net amount of $${result.netPrice.toFixed(2)} incurs $${result.taxAmount.toFixed(2)} in tax, resulting in a total price of $${result.totalPrice.toFixed(2)}.`,
      },
    );
  }, [direction, priceInput, taxRate, result]);
              ...
              <div>
                <label className="block text-xs font-semibold text-muted-foreground mb-1.5">
                  {direction === "forward"
                    ...
                    : "Total Receipt Amount Including Tax ($)"}
                </label>
                <input
                  type="number"
                  min="0"
