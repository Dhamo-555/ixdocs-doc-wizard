...
function TipCalculatorPage() {
  ...
  const [billAmount, setBillAmount] = useState<number>(65);
    ...
    return buildCalcReportInput(
      ...
      `$${result.totalBill.toFixed(2)}`,
      {
        metrics: [
          ...
          { label: "Total per Person", value: `$${result.totalPerPerson.toFixed(2)}` },
          { label: "Tip per Person", value: `$${result.tipPerPerson.toFixed(2)}` },
        ],
        formula: "Tip = Bill × (Tip% / 100); Total per Person = (Bill + Tip) / People",
        explanation: `On a bill of $${billAmount.toFixed(2)} with an ${tipPercent}% tip ($${result.tipAmount.toFixed(2)}), split among ${splitCount} diners, each person pays $${result.totalPerPerson.toFixed(2)}.`,
      },
    );
  }, [billAmount, tipPercent, splitCount, result]);
